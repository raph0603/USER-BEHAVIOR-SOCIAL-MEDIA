...
def _metrics(y_true, proba, threshold: float) -> dict:
    return {
        ...
        "f1": round(float(f1_score(y_true, (proba >= threshold).astype(int), zero_division=0)), 3),
...
def main() -> None:
    ...
    parser.add_argument("--output", "--evaluation-output", type=Path, default=DEFAULT_OUTPUT)
    ...
    result = {
        ...
        **observed_lineage,
        ...
        "evaluation": {
            "split": "persisted split_name or GroupShuffleSplit by author_hash",
            "seed": args.seed,
