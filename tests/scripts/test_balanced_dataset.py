...
class BalancedDatasetTests(unittest.TestCase):
    def test_balancing_sources_are_syntactically_valid(self):
        paths = [
            ROOT / "spark" / "jobs" / "maintenance" / "build_balanced_dataset.py",
            ROOT / "orchestrator" / "dags" / "build_balanced_comment_dataset.py",
    ...
    def test_balancing_dag_runs_spark_job_with_pipeline_lock(self):
        source = (ROOT / "orchestrator" / "dags" / "build_balanced_comment_dataset.py").read_text(
            encoding="utf-8"
        )
